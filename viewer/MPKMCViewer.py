...
MCSGoDialog
""" 
class MCSGoDialog(QtWidgets.QDialog):
...
SetDispDialog
""" 
class SetDispDialog(QtWidgets.QDialog):
  def __init__(self, parent, kmc, draw):
    QtWidgets.QDialog.__init__(self, parent)
    self.draw = draw
    self.setWindowTitle("Set Display")
    vbox = QtWidgets.QVBoxLayout(self)
    self.table = QtWidgets.QTableWidget(draw.ntypes, 3)
    self.table.setHorizontalHeaderLabels(["Type", "Display", "Diameter"])
    for i in range(draw.ntypes):
      self.table.setItem(i, 0, QtWidgets.QTableWidgetItem(str(draw.types(i))))
      ...
      self.table.setItem(i, 2, QtWidgets.QTableWidgetItem(str(draw.get_dia(i))))
...
SetShiftDialog
""" 
class SetShiftDialog(QtWidgets.QDialog):
  def __init__(self, parent, kmc, draw):
    QtWidgets.QDialog.__init__(self, parent)
    self.draw = draw
...
class EventPlotDialog(QtWidgets.QDialog):
  def __init__(self, parent, kmc):
    QtWidgets.QDialog.__init__(self, parent)
...
SearchTableDialog
"""
class SearchTableDialog(QtWidgets.QDialog):
  def __init__(self, parent, glwidget):
    ...
    button1 = QtWidgets.QPushButton('Search')
